...
function renderProductos() {
  ...
  filtered.forEach(prod => {
    ...
    html += `<div class="card" data-id="${prod.id}"><div class="card-img">${imgBlock}<div class="badge-gen"> Inspiraciones Premium</div></div><div class="card-body"><div class="card-genero">${generoLabel}</div><div class="card-nombre">${escapeHtml(prod.nombre)}</div><div class="card-precio-preview"><small>L.</small> 250 <small style="font-size:0.65rem;opacity:.7">50ml</small></div><button class="btn-ver-detalle" data-id="${prod.id}"> Ver acordes y agregar</button></div></div>`;
  });
...
// Interceptar renderProductos para volver a observar tarjetas
