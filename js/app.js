...
function renderCartSidebar() {
  ...
  if (cart.length === 0) {
    ...
    cartTotalSpan.innerHTML = 'L. 0';
    const subtotalEl = document.getElementById('cartSubtotalPrice');
    if (subtotalEl) subtotalEl.innerText = 'L. 0';
    return;
  }
  let html = '', total = 0;
  cart.forEach((item, idx) => {
    ...
    const emoji = item.genero === 'caballero' ? '👔' : '👗';
...
function addAllFavsToCart() {
  ...
  favorites.forEach(prod => { addToCart(prod, 50, 1); });
...
function sendOrderToWhatsapp() {
  ...
  let mensaje = "🌿 *NUEVO PEDIDO - GOTA MAESTRA HN* 🌿\n\n";
  let total = 0;
  cart.forEach(item => {
    const subtotal = item.cantidad * item.precio;
    total += subtotal;
  ...
  mensaje += `\n🚚 _Nota: El costo de envío se calculará por separado y no está incluido en el total._\n\n¡Gracias por tu pedido! 🙏`;
  ...
  const orderData = { items: cart.map(item => ({ id: item.id, nombre: item.nombre, genero: item.genero, ml: item.ml, cantidad: item.cantidad, precio: item.precio })), total: total };
  fetch(SHEET_URL, { method: 'POST', mode: 'no-cors', body: new Blob([JSON.stringify(orderData)], { type: 'application/json' }) }).catch(err => console.log("Error Sheets:", err));
...
function setupSearchHandlers() {
  // Para el searchInput normal (desktop)
  if (searchInput) {
    searchInput.addEventListener('focus', () => {
      // Al hacer focus, aseguramos que no haya scroll automático no deseado
      setTimeout(() => {
...
// Override renderProducts to re-observe cards
...
window.addEventListener('popstate', (e) => {
  ...
  if (cartSidebar.classList.contains('open')) { closeCartSidebar(true); }
